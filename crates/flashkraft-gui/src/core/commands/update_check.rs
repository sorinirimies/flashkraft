...
//!
//! Lives in the GUI crate (not `flashkraft-core`) so the `reqwest` HTTP
//! client — and its `ring` TLS backend, which needs a C compiler for the
//! target — never gets pulled into the cross-compile-checked core crate.
