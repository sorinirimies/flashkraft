...
#   3. Quality gate: fmt → clippy → tests → cross-compile checks
#   4. Commit strategy:
...
#   nu scripts/upgrade_deps.nu --skip-cross   # skip cross-compile checks (local runs)
...
def run_check [
    ...
    action: closure  # Command to run, e.g. { run-external "cargo" "fmt" "--check" }
] {
...
def main [
    ...
    --skip-cross (-S)                                                      # Skip cross-compile checks (for local runs without mingw-w64)
    ...
    print $"($cyan)════════════════════════════════════════($reset)"
    print $"($cyan)  FlashKraft — Nightly Dependency Upgrade($reset)"
    print $"($cyan)════════════════════════════════════════($reset)"
    print ""

    # ── Phase 1: Upgrade Cargo.toml pins ─────────────────────────────────────
    # cargo-edit's `cargo upgrade` rewrites the version strings in Cargo.toml
    # (including [workspace.dependencies]).  --incompatible allow lifts the
    # major-version guard so `dirs = "5.0"` can move to "6.0", etc.
    ...
    let fmt    = (run_check "fmt    " { run-external "cargo" "fmt" "--all" "--" "--check" })
    let clippy = (run_check "clippy " { run-external "cargo" "clippy" "--workspace" "--all-targets" "--all-features" "--" "-D" "warnings" "-A" "deprecated" })
    let tests  = (run_check "tests  " { run-external "cargo" "test" "--workspace" "--all-features" "--all-targets" })

    let cross = if $skip_cross {
        print $"  ($yellow)cross-checks skipped \(--skip-cross\)($reset)"
        [true true true]
    } else {
        [
            (run_check "cross win     " { run-external "cargo" "check" "-p" "flashkraft-core" "--target" "x86_64-pc-windows-gnu" })
            (run_check "cross mac/x86 " { run-external "cargo" "check" "-p" "flashkraft-core" "--target" "x86_64-apple-darwin" })
            (run_check "cross mac/arm " { run-external "cargo" "check" "-p" "flashkraft-core" "--target" "aarch64-apple-darwin" })
        ]
    }

    let gate_passed = (all_passed ([$fmt $clippy $tests] | append $cross))
    ...
    if $toml_changed and (not $gate_passed) {
        ...
        print "    • Lower the version pin in [workspace.dependencies] in Cargo.toml."
        exit 1
    }
