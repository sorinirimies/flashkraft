...
def "test all passed six true mirrors full quality gate" [] {
    # Mirrors: fmt clippy tests cross_win cross_mac_x86 cross_mac_arm
    let gate = [true true true true true true]
    assert (all_passed $gate)
}

def "test all passed six with fmt failure" [] {
    let gate = [false true true true true true]
    assert not (all_passed $gate)
}

def "test all passed six with cross failure" [] {
    let gate = [true true true false true true]
    assert not (all_passed $gate)
}
...
def "test no commit when gate passes but nothing dirty" [] {
    let gate_ok = (all_passed [true true true true true true])
    let label   = (commit_label false false "2026-03-24")
    assert $gate_ok
    assert ($label | is-empty)
}
...
def "test revert path when gate fails and toml dirty" [] {
    ...
    let gate_ok     = (all_passed [true false true true true true])
...
def "test no revert needed when gate fails but toml not changed" [] {
    let gate_ok      = (all_passed [false true true true true true])
