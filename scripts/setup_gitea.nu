...
#   nu scripts/setup_gitea.nu git@gitea.example.com:user/flashkraft.git
#   nu scripts/setup_gitea.nu https://gitea.example.com/user/flashkraft.git
...
def main [
    ...
    if $git_check.exit_code != 0 {
        fail "Not a git repository. Please run this script from the flashkraft directory."
    }

    print ""
    print $"($cyan)════════════════════════════════════════($reset)"
    print $"($cyan)  FlashKraft — Gitea Remote Setup($reset)"
    print $"($cyan)════════════════════════════════════════($reset)"
    print ""
