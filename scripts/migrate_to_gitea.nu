...
#   nu scripts/migrate_to_gitea.nu --gitea-url git@gitea.example.com:user/flashkraft.git
...
def main [
    ...
    let resolved_url = if ($gitea_url | is-empty) {
        ...
        print "  SSH  example: git@gitea.example.com:user/flashkraft.git"
        print "  HTTPS example: https://gitea.example.com/user/flashkraft.git"
        print ""
    ...
    print $"  ($cyan)just release 0.9.0($reset)         # Release to GitHub"
    print $"  ($cyan)just release-gitea 0.9.0($reset)   # Release to Gitea"
    print $"  ($cyan)just release-all 0.9.0($reset)     # Release to both remotes"
    print ""
